...
const semanas = decks.filter(d => d.name !== 'openclass-gestionseguridad');
...
const names = ['portal', ...semanas.map(d => d.name.replace('gestionseguridad_', ''))].join(',');
...
semanas.forEach((deck, i) => {
  console.log(`   ${deck.name.replace('gestionseguridad_', '')}  → http://localhost:${BASE_PORT + i + 1}`);
});
console.log('');
